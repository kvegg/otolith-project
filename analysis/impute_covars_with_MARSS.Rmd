...
# mod_list$c <- matrix(sin(2 * pi * seq(nrow(cobs_m)) / n_days), nrow = 1)
## Q
...
if(!file.exists(file.path(analdir, "temp_imputed.rds"))) {
  tmp <- MARSS(y = t(cobs_m), model = mod_list, control = list(maxit = 2000))
  ## save results to file
  saveRDS(tmp, file.path(analdir, "temp_imputed.rds"))
}
```
