...
readRDS(file.path(analdir, "base_temp_model.rds"))
```
...
if(!file.exists(file.path(analdir, "rw_temp_model.rds"))) {
  fit_base <- MARSS(y = t(cobs_m), model = mod_list, control = list(maxit = 2000))
  ## save results to file
  saveRDS(fit_base, file.path(analdir, "rw_temp_model.rds"))
}
```
...
```{r examine_rw_model}
readRDS(file.path(analdir, "rw_temp_model.rds"))
```
...
intvl <- nrow(cobs) / (n_yrs - 0.5)
## discrete cosine wave
cc <- -cos(2 * pi * seq(nrow(cobs)) / intvl)
par(mai=c(0.9,0.9,0.1,0.1), omi=c(0,0,0,0))
plot.ts(cc, ylab = "Cosine", xaxt="n")
axis(side = 1,
